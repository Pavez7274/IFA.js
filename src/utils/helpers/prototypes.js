...
const fs = require("fs");
String.prototype.toBoolean = function (_returnNone = true, _return = false, _force = false) {
	return ['yes', 'true', '!0', 'si'].some(_ => _ === this.toLowerCase())
		? true : _force
			? false : this.replace(/(!1|false|no|void 0|void\(0\)|undefined)/gim, '').trim().length 
				? true : _returnNone ? 'none' : _return
}
...
String.prototype.toObject=function(){
	return JSON.parse(eval(`JSON.stringify(${this})`))
}
...
Array.prototype.deleteAll=function(k){
	var f = new Array()
	for(var _ of this){
		if(_===k)continue;
		f.push(_)
