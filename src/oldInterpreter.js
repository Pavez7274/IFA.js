const IF=require('./utils/helpers/if.js'),Discord=require('discord.js'),{CustomFunction}=require('./classes/Functions.js'),AoiError=require('./classes/AoiError.js'),Util=require('./classes/Util.js'),{Time}=require('./utils/helpers/customParser.js'),{CheckCondition}=require('./utils/helpers/checkCondition.js'),{mustEscape}=require('./utils/helpers/mustEscape.js'),{Command}=require('./classes/Commands.js');
/**
 ...
 */
const Interpreter=async(client,message,args,command,_db,returnCode=!1,channelUsed,data={},useChannel,returnMessage,returnExecution,returnID,sendMessage=!1) => {
	try{
		//defining vars//
    ...
    let anErrorOccuredPlsWait,embeds,deleteIn,suppressErrors,editIn=undefined,error,attachments=[],components=[],reply,allowedMentions=disableMentions,FuncData,msgobj,funcLine,returnData={},funcs
      ...
      console.time(`interpreter-${start}`) 
		...
		for(let i=0;T>i;i++){
			funcs=client.functionManager.prioritizeFunction(code,'$nonEscape',i-1)
		}
		var T2=funcs.filter(f=>f==='$if').length
		for(let i=0;T2>i;i++){
			funcs=client.functionManager.prioritizeFunction(code,'$if',i-1)
		}
