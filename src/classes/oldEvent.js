...
const Execute = async (client, command, eventdata) => {
  const data = {author:undefined,member:undefined,guild:undefined}
  const channel = client.channels.cache.get(eventdata.channel)
	data.channel = channel; data.object = eventdata
	await interpreter(client, {}, [], command, undefined, false, data.channel, data);
};
