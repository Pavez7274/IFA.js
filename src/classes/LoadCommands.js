...
class LoadCommands {
    ...
    async load(client, path, debug = true) {
        ...
        for (const {name} of dirents) {
            ...
            debugs.push(
                `|${this.colors?.walking||""}Walking in ${name}${this.allColors.reset|| ""}`,
            );
...
|-------------------------------------------|`,
                ...
                debugs.push(`|${this.colors.loaded?.command||""}'${cmd.name||cmd.channel}'${this.allColors.reset||""} |${this.colors.loaded?.type||""}${cmd.type} ${this.allColors.reset||""}|${this.colors.loaded?.text||""}Loaded${this.allColors.reset||""} |
|------------------------------------------|`);
            }
        }
