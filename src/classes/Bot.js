...
const OldCustomEvent = require('./oldEvent.js')

class Client extends BaseClient {
    constructor(options) {
        super(options);
				this._distube=new DisTube(this,this.aoiOptions.distube);
        ...
        if(this.aoiOptions.respondOnEdit){
				...
				this.giveawaysManager=new GiveawaysManager(this,this.aoiOptions.giveaways||{storage: './database/giveaways.json'})
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		createCustomEvent(EventEmitter) {
			return new OldCustomEvent(this, EventEmitter);
		}

		onAddSong() {
			this._distube.on('addSong', async (q,s) => {
				const cmds = this.cmd.addSong.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q,song:s}})
				}
			})
		}
		addSongCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.addSong.size} needs a code!`);
				this.cmd.addSong?.set(this.cmd.addSong.size, new Command(d, this));
      }
		}

		onAddList() {
			this._distube.on('addList', async (q,p) => {
				const cmds = this.cmd.addList.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q,playlist:p}})
				}
			})
		}
		addListCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.addlist.size} needs a code!`);
				this.cmd.addList?.set(this.cmd.addList.size, new Command(d, this));
      }
		}

		onDeleteQueue(){
			this._distube.on('deleteQueue', async q => {
				const cmds = this.cmd.deleteQueue.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q}})
				}
			})
		}
		deleteQueueCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.deleteQueue.size} needs a code!`);
				this.cmd.deleteQueue?.set(this.cmd.deleteQueue.size, new Command(d, this));
      }
		}

		onDisconnect(){
			this._distube.on('disconnect', async q => {
				const cmds = this.cmd.disconnect.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q}})
				}
			})
		}
		disconnectCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.disconnect.size} needs a code!`);
				this.cmd.disconnect?.set(this.cmd.disconnect.size, new Command(d, this));
      }
		}

		onEmpty(){
			this._distube.on('empty', async q => {
				const cmds = this.cmd.empty.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q}})
				}
			})
		...
		emptyCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.empty.size} needs a code!`);
				this.cmd.empty?.set(this.cmd.empty.size, new Command(d, this));
      }
		}

		onDistubeError(){
			this._distube.on('error', async (q,e) => {
				const cmds = this.cmd.distubeError.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{channelID:q.textChannel.id,error:e}})
				}
			})
		}
		distubeErrorCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.disrubeError.size} needs a code!`);
				this.cmd.disrubeError?.set(this.cmd.disrubeError.size, new Command(d, this));
      }
		}

		onFinish(){
			this._distube.on('finish',async q=>{
				const cmds = this.cmd.finish.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this,{},[],{code:cmd.code},this.db,!1,'',{vars:{queue:q}})
				}
			})
		}
		finishCommand(...data){
			for(const d of data){
        if(!d.code)throw new TypeError(`Command ${this.cmd.finish.size} needs a code!`);
				this.cmd.finish?.set(this.cmd.finish.size,new Command(d,this));
      }
		}

		onFinishSong() {
			this._distube.on('finishSong', async (q,s) => {
				const cmds = this.cmd.finishSong.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q}})
				}
			})
		}
		finishSongCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.finishSong.size} needs a code!`);
				this.cmd.finishSong?.set(this.cmd.finishSong.size, new Command(d, this));
      }
		}

		onNoRelated(){
			this._distube.on('noRelated', async q => {
				const cmds = this.cmd.noRelated.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q}})
				}
			})
		}
		noRelatedCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.noRelated.size} needs a code!`);
				this.cmd.noRelated?.set(this.cmd.noRelated.size, new Command(d, this));
      }
		}

		onPlaySong() {
			this._distube.on('playSong', async (q,s) => {
				const cmds = this.cmd.playSong.allValues()
				for(const cmd of cmds) {
					await require('../interpreter.js')(this, {}, [], {code:cmd.code}, this.db, false, '', {vars:{queue:q,song:s}})
				}
			})
		}
		playSongCommand(...data) {
			for (const d of data) {
        if (!d.code) throw new TypeError(`Command ${this.cmd.playSong.size} needs a code!`);
				this.cmd.playSong?.set(this.cmd.playSong.size, new Command(d, this));
      }
		}
